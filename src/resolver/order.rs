    ...
    pub fn resolve(&self, name: &str) -> Result<ResolveResponse> {
            ...
            if let Some(p) = alias_map::resolve_alias(&map, name) {
                ...
                if allowed {
                    return Ok(ResolveResponse {
                        ...
                        resolved_id: Some(format!("axiomregent.repo.{}", name.replace('/', "."))),
        ...
        for repo_dir in &candidates {
            if let Some(url) = git_remote::read_origin_url(&self.fs, repo_dir)?
                && git_remote::remote_matches(name, &url)
            {
        ...
        Ok(ResolveResponse {
            ...
            fix_hint: Some(format!(
                "Add an alias in ~/.axiomregent/mcp-aliases.json for '{}' to pin the correct path.",
                name
            )),
        })
    }
...
mod tests {
    ...
    fn test_resolver_alias_security_containment() {
        ...
        fs.add_file(
            "/User/bart/.axiomregent/mcp-aliases.json",
            r#"{
            "hack": "/etc/secret"
        }"#,
        );

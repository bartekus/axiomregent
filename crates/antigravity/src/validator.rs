...
impl Validator {
    ...
    fn do_validate<C: McpClient>(changeset_path: &Path, client: &C) -> Result<ChangesetStatusV1> {
        ...
        // 4. MCP Checks
        // Preflight
        match client.preflight("standard") {
        ...
        // Impact
        // Extract changed paths from plan
        let mut changed_paths = Vec::new();
        for task in &plan.tasks {
            for tool_call in &task.tool_calls {
                // Heuristic: if tool is write_file, path is in args["path"] or args is string?
                // Checking usage in other files: arguments is Value (object).

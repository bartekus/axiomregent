...
use antigravity::agent::{AgentConfig, generate_changeset};
use antigravity::executor::Executor;
use antigravity::schemas::{LockFile, PlanTask, ToolCall};
use antigravity::validator::{McpClient, Validator};
use anyhow::Result;
use std::fs;
...
impl McpClient for MockMcp {
    fn preflight(&self, _mode: &str) -> Result<bool> {
        Ok(true)
    }
...
fn create_config() -> AgentConfig {
    let task = PlanTask {
        ...
        tool_calls: vec![ToolCall {
            ...
            arguments: serde_json::json!({"file": "foo.txt"}),
        }],
    };
...
fn test_end_to_end_flow() -> Result<()> {
    ...
    let final_status: antigravity::schemas::ChangesetStatusV1 =
...
fn test_multiple_tool_calls_counting() -> Result<()> {
    ...
    config.tasks[0].tool_calls.push(ToolCall {
        tool_name: "write_file".to_string(),
        arguments: serde_json::json!({"file": "bar.txt"}),
    });
...
fn test_post_execution_drift_failure() -> Result<()> {
    ...
    let status_bytes = fs::read(path.join("05-status.json"))?;
    let status: antigravity::schemas::ChangesetStatusV1 = serde_json::from_slice(&status_bytes)?;
...
impl McpClient for LockInspectionMcp {
    ...
    fn call_tool(&self, name: &str, _args: &serde_json::Value) -> Result<serde_json::Value> {
        ...
        let canonical_bytes = antigravity::canonical::to_canonical_json(&lock)?;
        assert_eq!(
